...
ssh_port_host=2222
cur_dir=$(pwd)
fedora_iso_base_url='https://dl.fedoraproject.org/pub/fedora/linux/releases/43/Everything/aarch64/iso/'
fedora_iso='Fedora-Everything-netinst-aarch64-43-1.6.iso'
...
if [[ -n $cdrom ]] && [[ ! -f $fedora_iso ]]; then
    ...
    # e.g. if fedora_iso_base == Fedora-Everything-netinst-aarch64-43.iso,
    # it discovers the most recent Fedora-Everything-netinst-aarch64-43*.iso
